...
(function ($) {
    ...
    $.widget("ui.listerFactures", {
		...
		_lister: function(page, search) {
			...
			$.post('ajax/factures/factures.lister', post, function(data) {
				if(data) {
		...
		_afficher_factures: function(data) {
			var that = this;
			...
			$('<a target="_blank" href="pdf/factures.php?id='+ data.id +'"><button>Imprimer</button></a>').appendTo(td);
			...
			.click(function() {
				...
				$(that.element).ajouterFactures({data:data.id});
			});
			...
			.click(function() {
				...
				$('<p>Supprimer Facture n° ' + data.id + ' ?</p>').appendTo(fenetre);
				...
				.click(function() {
					$(fenetre).remove();
					$('#' + data.id).remove();
					$.post('ajax/factures/factures.supprimer', {id:data.id});
				});
			});
		},
